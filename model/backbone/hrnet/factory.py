...
from model.backbone.hrnet.config import MODEL_CONFIGS
from model.backbone.hrnet.hrnet_core import HighResolutionNet

logger = logging.getLogger('hrnet_factory')
...
def _hrnet(arch: str, pretrained: bool = True, progress: bool = True, **kwargs):
    cfg = MODEL_CONFIGS[arch]
    model = HighResolutionNet(cfg, **kwargs)
    if pretrained:
        url = MODEL_URLS.get(arch, None)
        if url is None:
            logger.warning(f"No pretrained URL for {arch}; loading uninitialized weights.")
        else:
            state_dict = load_state_dict_from_url(url, progress=progress, map_location='cuda:0')
