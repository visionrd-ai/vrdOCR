...
from src.model import Swin_TransformerDecoder
from src.dataset import get_swin_dataloader
from src.char_tokenizer import CharacterLevelTokenizer
import logging
import os
from datetime import datetime
...
wandb.init(project="ocr-training", name=f"swin_transformer_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
for epoch in range(EPOCHS):
    model.train()
    epoch_train_loss = 0.0
    epoch_train_metrics = {'batch_accs':[], 'batch_cers':[]}
    optimizer.zero_grad()
    ...
    wandb.log({"Train Loss": avg_epoch_train_loss, "Train Accuracy": avg_epoch_train_acc, "Train CER": avg_epoch_train_cer})
    
    logging.info(f"TRAIN | Epoch {epoch+1} - Loss: {avg_epoch_train_loss:.3f}, Acc: {avg_epoch_train_acc:.3f}%, CER: {avg_epoch_train_cer:.3f}")
    
    if (epoch + 1) % EVAL_EVERY_N_EPOCHS == 0:
        val_accuracy, val_cer, samples = beam_search_evaluate(epoch+1, model, test_loader, tokenizer, DEVICE, beam_size=5)
        
        wandb.log({"Val Accuracy": val_accuracy, "Val CER": val_cer})
        logging.info(f"EVAL | Epoch {epoch+1} - Accuracy: {val_accuracy:.3f}%, CER: {val_cer:.3f}")
