...
for epoch in range(EPOCHS):
    ...
    epoch_correct = 0
    epoch_total = 0
    epoch_cer = 0.0  # Initialize CER accumulator
    ...
    for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1} [Training]")):
        ...
        # - decoder_input_ids: All tokens except the last (e.g., [<s>, w1, w2, ..., wn-1])
        # - decoder_target_ids: All tokens except the first (e.g., [w1, w2, ..., wn, </s>])
        # -------------------------------
        ...
        # Calculate CER and accuracy for each sample in the batch.
        # Note: We use the shifted target tokens as ground truth.
        ...
        for j in range(outputs.size(0)):
            ...
            epoch_cer += calculate_cer(pred_text, gt_text)
